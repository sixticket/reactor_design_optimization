...
OUTPUT_FILE = f"./dpo_multi_lhs_seed{SEED}_results.csv"
...
# --- [LHS Target Generator] ---
def generate_lhs_targets(num_steps, min_val, max_val, bins):
    """
    1D Latin Hypercube Sampling을 통해 num_steps 만큼의 타겟 K_eff 생성.
    각 bin에서 공평하게 난수를 추출한 뒤 셔플링하여 반환.
    """
    targets = []
    bin_edges = np.linspace(min_val, max_val, bins + 1)
    
    while len(targets) < num_steps:
        # 1. 각 bin에서 난수 하나씩 추출 (10개)
        epoch_samples = [np.random.uniform(bin_edges[i], bin_edges[i+1]) for i in range(bins)]
        # 2. 순서 셔플
        random.shuffle(epoch_samples)
        targets.extend(epoch_samples)
        
    return targets[:num_steps]
...
def evaluate_grid(grid_str, target_k):
    ...
    try:
        ...
        # 🚨 Multi Target Fitness: 이번 스텝에 LHS로 뽑힌 target_k 와의 절대 거리 페널티
        penalty_weight = 100.0
        penalty_k = penalty_weight * abs(k_eff - target_k)
...
def run_online_dpo(model, tokenizer, num_steps, output_file):
    ...
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            "step", "target_k", "simulation_count", "elapsed_time_sec",
    ...
    # 🚨 DPO 스텝 수만큼 LHS 타겟 배열 생성
    lhs_targets = generate_lhs_targets(num_steps, LHS_MIN, LHS_MAX, LHS_BINS)

    pbar = tqdm(total=num_steps, desc="Online DPO (Multi-Target LHS)", unit="step")

    for step in range(1, num_steps + 1):
        # 🚨 이번 스텝의 타겟 K_eff 가져오기
        current_target_k = lhs_targets[step - 1]
        
        prompt = create_prompt(current_target_k, TARGET_FQ, TARGET_FDH)

        grid1 = generate_grid(model, tokenizer, prompt, temperature=1.0)
        grid2 = generate_grid(model, tokenizer, prompt, temperature=1.0)

        metrics1 = evaluate_grid(grid1, current_target_k)
        metrics2 = evaluate_grid(grid2, current_target_k)
        sim_count += 2
        ...
        with open(output_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                step, current_target_k, sim_count, round(elapsed, 2),
        ...
        pbar.update(1)
        pbar.set_postfix({
            "Target_K": f"{current_target_k:.3f}",
            "Chosen_K": f"{chosen_metrics[1]:.3f}",
            "Gd": chosen_metrics[4],
            "Loss": f"{loss_val:.4f}"
        })
...
if __name__ == "__main__":
    ...
    print(f"=" * 60)
    print(f"⚙️  Mode: MULTI-TARGET (LHS Sampling {LHS_MIN} ~ {LHS_MAX}, {LHS_BINS} bins)")
    ...
    total_sims = run_online_dpo(
        model, tokenizer,
        num_steps=NUM_STEPS,
        output_file=OUTPUT_FILE
    )

    final_path = f"./dpo_optimized_multi_lhs_seed{SEED}_model"
    model.save_pretrained(final_path)
    tokenizer.save_pretrained(final_path)

    print("\n" + "=" * 60)
    print("🏆 Online DPO Optimization (Multi-LHS) Complete!")

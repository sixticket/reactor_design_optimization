...
OUTPUT_FILE = f"./grpo_sft_only_multi_seed{SEED}_results.csv"
FINAL_MODEL_DIR = f"./grpo_sft_only_multi_seed{SEED}_model"
...
# LHS Multi Target 설정
LHS_MIN, LHS_MAX, LHS_BINS = 1.0, 1.1, 10
...
if __name__ == "__main__":
    ...
    sim_count = 0
    pbar = tqdm(total=NUM_STEPS, desc="GRPO SFT-Only (Multi)")
    
    for step in range(1, NUM_STEPS + 1):
        current_target_k = lhs_targets[step - 1]
        prompt = f"Reactor Core Design (k={current_target_k:.5f}, fq={TARGET_FQ:.4f}, fdh={TARGET_FDH:.4f}):\n"
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=GROUP_SIZE) as executor:
            metrics = list(executor.map(evaluate_grid, grids, [current_target_k]*GROUP_SIZE))
        ...
        with open(OUTPUT_FILE, 'a', newline='') as f:
            ...
            for i in range(GROUP_SIZE):
                m = metrics[i]
                writer.writerow([step, current_target_k, i, sim_count, round(time.time()-start_time, 2), grids[i], round(m[0], 4), round(m[1], 5), round(m[2], 4), round(m[3], 4), m[4], round(loss_val, 4)])

        pbar.update(1); pbar.set_postfix({"k": f"{current_target_k:.3f}", "Fit": f"{best[0]:.3f}", "Gd": best[4]})

    pbar.close(); model.save_pretrained(FINAL_MODEL_DIR); tokenizer.save_pretrained(FINAL_MODEL_DIR)
    print("✅ SFT-Only Multi GRPO Complete!")
...
# --- [Configuration] ---
MODEL_ID = "google/gemma-3-270m"  # 🚨 원본 베이스 모델 (CPT 없음)
DATA_PATH = os.path.join(PROJECT_ROOT, "data_generation/reactor_10k_final.csv")
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "only_sft/sft")

LOG_FILE = os.path.join(OUTPUT_DIR, f"training_log_sft_only_seed{SEED}.csv")
MAX_LENGTH = 384
...
test_dataset.to_json(os.path.join(OUTPUT_DIR, f"test_dataset_sft_only_seed{SEED}.json"))
...
print(f"🤖 Loading base model for Full Fine-Tuning: {MODEL_ID}...")
...
final_path = os.path.join(OUTPUT_DIR, f"final_sft_only_model_seed{SEED}")
trainer.save_model(final_path)
tokenizer.save_pretrained(final_path)
print(f"🎉 SFT Ablation Complete! Logs saved to {LOG_FILE}")
